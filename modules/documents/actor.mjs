...
export default class Actor extends foundry.documents.Actor
{
	...
	/**
	 * Upon change to the party's fortune pool, if its number of tokens equals the number of members, triggers fortune refreshment.
	...
	async refreshFortune()
	{
		for(const member of this.system.members) {
			...
			if(owningUser || backupUser) {
				const buttons = [{
					action: "fortuneToken",
					label: "PARTY.DIALOG.fortuneRefreshment.BUTTONS.recoverFortuneToken"
				}, {
					action: "removeRechargeToken",
					label: "PARTY.DIALOG.fortuneRefreshment.BUTTONS.removeRechargeToken"
				}];
				...
				foundry.applications.api.DialogV2.query(
					...
					"wait", {
					buttons,
					content: game.i18n.localize("PARTY.DIALOG.fortuneRefreshment.description"),
					window: {title: "PARTY.DIALOG.fortuneRefreshment.title"},
					submit: async (result) => {
						if(result === "fortuneToken")
							await memberActor.adjustFortune(1);
						else if(result === "removeRechargeToken")
							await wfrp3e.applications.apps.selectors.RechargingCardSelector.wait();
						else
